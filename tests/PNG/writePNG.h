#ifndef MRC_WRITE_PNG
#define MRC_WRITE_PNG

#include <fstream>
#include <vector>


template<typename T> inline void write_to_stream(std::ofstream& stream,const T& t) { stream.write(reinterpret_cast<const char*>(&t),sizeof(T)); } // writes a variable t to a stream
...
/*
    Save a RGB/RGBA image in PNG format without compression.
    stream :	Output stream (by default using file descriptor).
    w      :	Width of the image. (<16383)
    h      :	Height of the image.
    img    :	Image pixel data in 24-bit RGB or 32-bit RGBA format.
    alpha  : Whether the image contains alpha channel.
*/
void writePNG(std::ofstream& stream, unsigned w, unsigned h, const unsigned char *img, bool alpha=false)
{
	// if stream is a file it must be opened in std::ios::binary mode otherwise the file header is corrupted
	
	// 1) writes file header:
	unsigned char sig[8] = { 0x89,'P','N','G','\r','\n','\32','\n' }; // header starting combination (8-byte signature)
	write_to_stream(stream, sig);
	...
	write_chunk(stream, temp, "IHDR");
	
	// 2.2) Data "IDAT" chunk:
	temp.clear();
	unsigned ADLERa = 1, ADLERb = 0;
	temp.push_back(0x78); temp.push_back(1); // Deflate block begin (2 bytes)
	for(int y=0; y<h; y++){ // Each horizontal line makes a block for simplicity
		...
		for (int x=0; x<w*(alpha?4:3); x++){
        	...
        	ADLERa = (ADLERa + (*img)) % 65521; ADLERb = (ADLERb + ADLERa) % 65521;
        	img++; // next pointer
		}
	}
	__int32 Adler = (ADLERb << 16) | ADLERa; // Deflate block end with Adler (4 bytes)
	temp.push_back((Adler >> 24) & 0xFF);
	temp.push_back((Adler >> 16) & 0xFF);
	temp.push_back((Adler >>  8) & 0xFF);
	temp.push_back((Adler >>  0) & 0xFF);
	write_chunk(stream, temp, "IDAT");
	
	
	// 2.3) Image End chunk (0 length):
	temp.clear();
	write_chunk(stream, temp, "IEND");
